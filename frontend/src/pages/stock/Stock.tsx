...
import { AlertTriangle, Plus, Package, Tag, Building2, X, PackagePlus, FileSpreadsheet, FileText, Search, Printer, Trash2 } from 'lucide-react'
...
export default function Stock() {
  ...
  const getAllVal = (p: any, key: string) => {
    ...
    switch (key) {
      ...
      case 'stockValueCost': return p.stock * p.costPrice
      case 'stockValueSell': return p.stock * p.price
      default: return ''
    }
  }
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            <button onClick={() => {
              ...
              const rows = visibleAllProducts.map((p: any) => vc.map(c => {
                ...
                return (['price','costPrice','stockValueCost','stockValueSell'].includes(c.key)) && typeof v === 'number' ? fmt(v) : v
              }))
              ...
              <table className="w-full text-sm">
                ...
                {(allProducts?.length ?? 0) > 0 && (
                  <tfoot className="bg-gray-50 border-t font-semibold text-sm">
                    <tr>
                      <td colSpan={COLS_ALL.filter(c => visAll.has(c.key) && !['stockBoxes','stockRem','stockValueCost','stockValueSell','price','costPrice'].includes(c.key)).length + 1} className="px-4 py-3 text-gray-600">
