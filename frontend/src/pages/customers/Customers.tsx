...
import { Plus, Search, Edit2, ChevronRight, X, ShoppingBag, Calendar, FileSpreadsheet, FileText } from 'lucide-react'
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => {
            const headers = ['Tên khách hàng', 'SĐT', 'Email', 'Địa chỉ', 'Giá trị đơn hàng', 'Công nợ']
            const rows = (data?.data || []).map((c: any) => [c.name, c.phone || '', c.email || '', c.address || '', c.orderValue || 0, c.debt || 0])
          ...
          <button onClick={() => {
            const headers = ['Tên khách hàng', 'SĐT', 'Email', 'Địa chỉ', 'Giá trị đơn hàng', 'Công nợ']
            const rows = (data?.data || []).map((c: any) => [c.name, c.phone || '', c.email || '', c.address || '', fmt(c.orderValue || 0), c.debt > 0 ? fmt(c.debt) : '-'])
      ...
      <div className="card p-0 overflow-hidden overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b">
            <tr>
              {['Tên khách hàng', 'SĐT', 'Email', 'Địa chỉ', 'Giá trị đơn hàng',
                `DS cộng dồn (${monthLabel})`, `Lần mua (${monthLabel})`, 'Công nợ', 'Thao tác'].map(h => (
                <th key={h} className="px-4 py-3 text-left font-medium text-gray-600 whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y">
            {isLoading && <tr><td colSpan={9} className="text-center py-10 text-gray-400">Đang tải...</td></tr>}
            {!isLoading && !data?.data?.length && <tr><td colSpan={9} className="text-center py-10 text-gray-400">Không có khách hàng</td></tr>}
            {data?.data?.map((c: any) => (
              <tr key={c.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-blue-600 font-medium whitespace-nowrap">{fmt(c.orderValue || 0)}</td>
